...
int main()
{
	...
	S4_real Lr[4] = { 1, 0, 0, 0 };
	unsigned int nG = 1;
	unsigned int argflag = 0;
	S4_Simulation PS0;
	S4_Simulation *ps1 = &PS0;
	S4_Simulation** pS ;
	pS= &ps1;
	*pS = S4_Simulation_New(Lr, nG, NULL);
	...
	int type1 = S4_MATERIAL_TYPE_SCALAR_COMPLEX; // epsilon type
	S4_real eps1[2] =  {12, 0};
	S4_MaterialID M1 = S4_Simulation_SetMaterial(S, -1, name1, type1, eps1);
	std::cout << "Here AddMaterial: Silicon"  << std::endl;

    //AddMaterial("Vacuum", {1,0})
	const char *name2 = "Vacuum";
	int type2 = S4_MATERIAL_TYPE_SCALAR_COMPLEX; // epsilon type
	S4_real eps2[2] =  {1,0};
	S4_MaterialID M2 = S4_Simulation_SetMaterial(S, -1, name2, type2, eps2);
	...
	S4_real thickness1 = 0;
	const char *matname1 = "Vacuum";
	S4_MaterialID LM1 = S4_Simulation_GetMaterialByName(S, matname1);
	layer1 = S4_Simulation_SetLayer(S, -1, layername1, &thickness1, -1, LM1);
	std::cout << "Here AddLayer 1"  << std::endl;

	// AddLayer('Slab', 0.5, 'Vacuum')
	S4_LayerID layer2;
	const char *layername2 = "Slab";
	S4_real thickness2 = 0.5;
	const char *matname2 = "Vacuum";
	S4_MaterialID LM2 = S4_Simulation_GetMaterialByName(S, matname2);
	layer2 = S4_Simulation_SetLayer(S, -1, layername2, &thickness2, -1, LM2);
	...
	S4_LayerID patternlayer2 = S4_Simulation_GetLayerByName(S, layer_name2);
	double center2[2] = {0,0}, halfwidths2[2] = {0.25, 0.5};
    S4_MaterialID PM2 = S4_Simulation_GetMaterialByName(S, material_name2);
	S4_real angle2 = 0 / 360.;
	int ret2;
    if(0 == Lr[1] && 0 == Lr[2] && 0 == Lr[3]){
		ret2 = S4_Layer_SetRegionHalfwidths(S, patternlayer2, PM2, S4_REGION_TYPE_INTERVAL, halfwidths2, center2, &angle2);
		std::cout << "Here SetLayerPatternRectangle 2 : enter if"  << std::endl;
	}else{
		ret2 = S4_Layer_SetRegionHalfwidths(S, patternlayer2, PM2, S4_REGION_TYPE_RECTANGLE, halfwidths2, center2, &angle2);
	...
	S4_real thickness5 = 0;
	const char *copyname5 = "AirAbove";
	S4_LayerID Lcopy5 = S4_Simulation_GetLayerByName(S, copyname5);
	layer5 = S4_Simulation_SetLayer(S, -1, layername5, &thickness5, Lcopy5, -1);
	...
	//-------------------------------------Set Frequency--------------------------------------------- 
	// SetFrequency(0.64)
	S4_real freq[2] = {0.01, 0}; // real part and img part 
